...
const UPLOADS_DIR = options.cache;

if (!fs.existsSync(UPLOADS_DIR)) {
  console.log(`Uploads directory not found. Creating directory: ${UPLOADS_DIR}`);
...
app.get('/inventory/:id/photo', (req, res) => {
    ...
    if (!item || !item.photoUrl) {
        return res.status(404).json({ message: 'Photo not found for this item' });
    }
    const photoPath = path.join(__dirname, UPLOADS_DIR, path.basename(item.photoUrl));
    ...
    if (fs.existsSync(photoPath)) {
        ...
        res.status(404).json({ message: 'Photo file is missing from the server' });
    }
});
...
app.post('/search', (req, res) => {
    ...
    if (!item) {
        return res.status(404).send('<h2>Item not found by ID</h2>');
    }
...
app.listen(PORT, HOST, () => {
  ...
  console.log(`Uploads are stored in: ${UPLOADS_DIR}`);
